...
#let ieee(
  ...
  set text(font: "TeX Gyre Termes", size: text-size, spacing: .35em)
  ...
  show figure.where(kind: table): set text(size: text-small-size)
  ...
  show figure.where(kind: table): set table(
    fill: (x,y) => if x == 0 or y == 0 {silver.lighten(25%)}
  )
  show figure.caption: set text(size: text-small-size)
  ...
  show heading: it => {
    ...
    set text(text-size, weight: 400)
    if it.level == 1 {
      // First-level headings are centered smallcaps.
      // We don't want to number the acknowledgment section.
      let is-ack = it.body in ([Acknowledgment], [Acknowledgement], [Acknowledgments], [Acknowledgements])
      set align(center)
      set text(if is-ack { text-size } else { text-size }, weight: "bold")
      ...
      for i in range(calc.ceil(authors.len() / 3)) {
        ...
        grid(
          ...
          ..slice.map(author => align(center, {
            text(size: text-size, author.name)
